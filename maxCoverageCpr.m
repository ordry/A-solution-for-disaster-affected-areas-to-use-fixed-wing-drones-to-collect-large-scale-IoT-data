...
for i=1:32
    for j=1:32
        x(pointplace)=75+(i-1)*150;
        y(pointplace)=75+(j-1)*150;
        pointplace=pointplace+1;
    end
end
...
Q=12*[ones(1,round(rate(1)*N)),zeros(1,N-round(rate(1)*N))];
Q=Q+12*[zeros(1,round(rate(1)*N)),ones(1,round(rate(2)*N)),zeros(1,N-round(rate(1)*N)-round(rate(2)*N))];
Q=Q+12*[zeros(1,round(rate(1)*N)+round(rate(2)*N)),ones(1,N-round(rate(1)*N)-round(rate(2)*N))];
